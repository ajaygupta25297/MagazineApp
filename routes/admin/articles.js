  ...
  (async () => {
    try {
      ...
      if (searchString == undefined) {
        ...
        let contactsCount = await Contact.find({
          seen: false,
        }).countDocuments();
        res.render("articles", {
          articles,
        ...
        if (imageTypes.includes(files.image.type)) {
          mv(
            ...
            function (err) {
              ...
              (async () => {
                try {
                  let lastnum = await Articles.find({}, { num: 1 })
                    .sort({ _id: -1 })
                    .limit(1);
                  if (lastnum.length !== 0) {
                    let create = await Articles.create({
                      title: fields.title,
                  ...
                  if (lastnum.length !== 0 && result.length !== 0) {
                    const payload = JSON.stringify({
                      title: fields.title,
                      body: `${fields.content.substring(0, 30)}...`,
                      articleUrl: lastnum[0].num + 1,
                      logo: files.image.name,
                    });
                    ...
                    for (let i = 0; i < result.length; i++) {
                      subscription = await {
                        ...
                        expirationTime: result[i].expirationTime,
                      };
          ...
          if (imageTypes.includes(files.image.type)) {
            mv(
              ...
              function (err) {
                if (err) throw err;
                (async () => {
                  let update = await Articles.updateOne(
                    { _id: id },
                    {
                      title: fields.title,
                      content: fields.content,
